...
object SemiDefault extends AutoDerivation[SemiDefault]:
  ...
  def join[T](ctx: CaseClass[SemiDefault, T]): SemiDefault[T] =
    new SemiDefault[T] {
      def default = ctx.construct(p => p.default.getOrElse(p.typeclass.default))
    }

  override def split[T](ctx: SealedTrait[SemiDefault, T]): SemiDefault[T] =
    new SemiDefault[T] {
      def default = ctx.subtypes.head.typeclass.default
    }

  given string: SemiDefault[String] = new SemiDefault[String] {
    def default = ""
  }
