...
class SumsTests extends munit.FunSuite:
  ...
  test("decode a Person as an Entity") {
    val res = summon[Decoder[Entity]].decode(
      """dev.fshp.magnolia1.tests.SumsTests.Person(name=John Smith,age=32)"""
    )
  ...
  test("sealed trait typeName should be complete and unchanged") {
    ...
    assertEquals(res.full, "dev.fshp.magnolia1.tests.SumsTests.Color")
  }
  ...
  test(
    ...
    assert(
      clue(
        error
      ) contains "No given instance of type scala.deriving.Mirror.Of[dev.fshp.magnolia1.tests.SumsTests.Parent] was found for parameter x$1 of method derived in trait Derivation."
    )
    assert(
      clue(
        error
  ...
  test("derive all subtypes in complex hierarchy") {
    ...
    val pkg = "dev.fshp.magnolia1.tests.SumsTests.Complex"
    val expected = List(
      s"$pkg.ClassH",
      s"$pkg.Object",
      s"$pkg.ObjectC",
