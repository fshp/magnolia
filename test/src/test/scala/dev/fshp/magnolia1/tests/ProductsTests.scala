...
class ProductsTests extends munit.FunSuite:
  ...
  test("case class typeName should be complete and unchanged") {
    ...
    assertEquals(res.full, "dev.fshp.magnolia1.tests.ProductsTests.Fruit")
  }
  ...
  test("show chained error stack when leaf instance is missing") {
    ...
    assert(
      clue(error) contains """|No given instance of type dev.fshp.magnolia1.examples.Show[String,
                              |  Seq[dev.fshp.magnolia1.tests.ProductsTests.Event]] was found.""".stripMargin
    )
  }
  ...
  test("show chained error stack") {
    ...
    assert(
      clue(error) contains "No given instance of type dev.fshp.magnolia1.examples.Show[String, Seq[(Double, String)]] was found."
    )
  }
