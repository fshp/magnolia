...
import dev.fshp.magnolia1.examples.*
...
class OtherTests extends munit.FunSuite:
  ...
  test("show error stack") {
    ...
    """)
    assert(
      clue(error) contains "No given instance of type dev.fshp.magnolia1.examples.Show[String, Alpha] was found."
    )
  }
  ...
  test("not attempt to instantiate Unit when producing error stack") {
    ...
    """)
    assert(
      clue(error) contains "No given instance of type dev.fshp.magnolia1.examples.Show[String, Unit] was found."
    )
  }
  ...
  test("not attempt to derive instances for refined types") {
    ...
    assert(
      clue(
        error
      ) contains """|No given instance of type dev.fshp.magnolia1.examples.Show[String, Long &
                    |  dev.fshp.magnolia1.tests.OtherTests.Character.Tag] was found.""".stripMargin
    )
  }
...
//    assert(error contains "No given instance of type deriving.Mirror.Of[tests.dev.fshp.magnolia1.tests.WeekDay] was found for parameter x$1 of method derived in trait Derivation.")
//  }
